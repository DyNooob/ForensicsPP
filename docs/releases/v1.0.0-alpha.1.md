...
- 完整源码和源码压缩包通过 GitHub 仓库提供。

静态包应通过 GitHub 静态托管、Nginx、Apache、对象存储或其他静态服务器访问。现代浏览器通常会限制通过 `file://` 直接运行 ES Module、Worker 和 WASM。
...
- The complete source and source archives are provided through the GitHub repository.

Serve the static package with GitHub static hosting, Nginx, Apache, object storage, or another static server. Modern browsers commonly restrict ES modules, workers, and WASM under `file://`.

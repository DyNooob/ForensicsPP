...
页面启动时自动计算当前版本发布日期，若超过 90 天，在顶部显示「版本可能已过时」横幅，点击可跳转 GitHub Releases 下载新版。关闭后仅对当前会话生效——切换工具不会重新出现，刷新页面后会再次提示。判定逻辑完全由代码计算（发布日期 + 90 天 vs 当前日期），每次发版只需维护 `appReleaseDate` 一个常量。

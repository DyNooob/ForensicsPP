...
- `ForensicsPP-v0.8.0-static.zip`：构建好的纯静态版本，可部署到 GitHub Pages、Nginx、Apache、对象存储或其他静态托管服务。现代浏览器通常会限制通过 `file://` 直接运行 ES Module、Worker 和 WASM，因此不要直接双击 `index.html`。
...
- `ForensicsPP-v0.8.0-static.zip`: pre-built static files for GitHub Pages, Nginx, Apache, object storage, or another static host. Modern browsers commonly restrict ES modules, workers, and WASM under `file://`, so do not open `index.html` directly.

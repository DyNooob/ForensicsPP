...
export function SqliteTool({ t }: { t: (typeof copy)["zh"] }) {
  ...
  return (
    <div className={`tool-grid sqlite-browser-grid sqlite-browser-simple ${hasSqliteDb ? "has-sqlite" : "empty-sqlite"}`}>
      ...
      {hasSqliteDb && <>
        <ASegmentedGroup className="sqlite-page-tabs wide-panel" value={sqlitePage} selects="single" aria-label={english ? "SQLite workspace pages" : "SQLite 工作区分页"}>
          <ASegmentedButton value="data" onClick={() => setSqlitePage("data")}>{english ? "Data" : "数据"}</ASegmentedButton>
          <ASegmentedButton value="sql" onClick={() => setSqlitePage("sql")}>SQL</ASegmentedButton>
          <ASegmentedButton value="schema" onClick={() => setSqlitePage("schema")}>{english ? "Schema" : "结构"}</ASegmentedButton>
          <ASegmentedButton value="changes" onClick={() => setSqlitePage("changes")}>{english ? "Changes" : "修改"}{dirty ? " *" : ""}</ASegmentedButton>
        </ASegmentedGroup>

        {sqlitePage === "data" && <div className="sqlite-data-workspace wide-panel">
          <div className="tool-panel sqlite-table-browser-panel">
            <div className="panel-heading-row"><PanelTitle title={t.sqliteTables} /><span className="status-pill">{visibleTables.length}/{tables.length}</span></div>
          ...
          <div className="tool-panel sqlite-main-data-panel">
            ...
            <div className="sqlite-simple-controls">
              ...
              <AButton variant="outlined" disabled={!data.editable} onClick={startCreate}>{t.sqliteNewRow}</AButton>
            </div>
            <div className="table-scroll sqlite-data-scroll">
              {data.columns.length ? <table className="data-table sqlite-data-table sqlite-browse-table" style={{ "--sqlite-table-width": `${sqliteDataMinWidth}px` } as React.CSSProperties}>
                ...
                <tbody>{data.values.map((row, rowIndex) => <tr key={`${data.rowids[rowIndex] ?? rowIndex}`}>
                  <td className="sqlite-action-cell"><div className="button-row compact-buttons sqlite-row-action-buttons">{data.editable && <AButton variant="text" icon={<EditOutlined aria-hidden="true" />} aria-label={sqliteRowActions.edit} title={sqliteRowActions.edit} onClick={() => startEdit(rowIndex)} />}<AButton variant="text" icon={<CopyOutlined aria-hidden="true" />} aria-label={sqliteRowActions.copy} title={sqliteRowActions.copy} onClick={() => copySqliteRow(data, row)} />{data.editable && <Popconfirm title={`${t.sqliteDeleteRow}: rowid ${data.rowids[rowIndex] ?? rowIndex + 1}?`} okText={t.sqliteDeleteRow} cancelText={t.cancelEdit} okButtonProps={{ danger: true }} onConfirm={() => deleteRow(rowIndex)}><AButton variant="text" danger icon={<DeleteOutlined aria-hidden="true" />} aria-label={sqliteRowActions.delete} title={sqliteRowActions.delete} /></Popconfirm>}</div></td>
                  {data.columns.map((column, columnIndex) => { const value = row[columnIndex] ?? null; const active = selectedCell?.rowIndex === rowIndex && selectedCell.columnIndex === columnIndex; return <td className={active ? "active-cell" : ""} key={column} title={displaySqliteValue(value)} onClick={() => setSelectedCell({ rowIndex, columnIndex, column, rowid: data.rowids[rowIndex] ?? null, value })} onDoubleClick={() => { setSelectedCell({ rowIndex, columnIndex, column, rowid: data.rowids[rowIndex] ?? null, value }); if (data.editable && typeof data.rowids[rowIndex] === "number") setCellEditOpen(true); }}>{displaySqliteValue(value)}</td>; })}
                </tr>)}</tbody>
        ...
        {sqlitePage === "sql" && <div className="tool-panel wide-panel sqlite-simple-sql-panel">
          <div className="panel-heading-row"><PanelTitle title={t.sqliteSql} /><AButton variant="filled" disabled={!sql.trim()} onClick={runQuery}>{t.runSql}</AButton></div>

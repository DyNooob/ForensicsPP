...
export function TimestampTool({ t }: { t: (typeof copy)["zh"] }) {
  ...
  const batchEvents = React.useMemo(() => parseTimestampCandidates(batchInput, "timestamp batch"), [batchInput]);
  ...
  return (
    <div className={`tool-grid timestamp-workbench timestamp-simple-workbench ${hasInput || batchInput.trim() ? "has-timestamp" : "empty-timestamp"}`}>
      ...
      {page === "single" && <>
        ...
        {hasInput && <div className="tool-panel wide-panel timestamp-simple-result-panel">
          ...
          {selectedCandidate ? <div className="table-scroll compact-scroll"><table className="data-table timestamp-conversion-table"><thead><tr><th>{english ? "Format" : "格式"}</th><th>{english ? "Value" : "值"}</th><th>{t.copy}</th></tr></thead><tbody>
            {conversions.map(([label, value]) => <tr key={`${label}-${value}`}><td>{label}</td><td className="mono-cell">{value}</td><td><AButton variant="text" disabled={!value || value === "--"} onClick={() => void navigator.clipboard.writeText(value)}>{t.copy}</AButton></td></tr>)}
          </tbody></table></div> : <div className="empty-state error-state">{rows[0]?.[1] || (english ? "Unsupported timestamp" : "无法识别时间戳")}</div>}
        </div>}

        {candidates.length > 1 && <div className="tool-panel wide-panel timestamp-simple-candidates-panel">
          <ToolPanelHeader title={t.timestampCandidates} subtitle={english ? `${candidates.length} possible interpretations` : `${candidates.length} 个可能解释`} />
      ...
      {page === "batch" && <div className="tool-panel wide-panel timestamp-simple-batch-panel">
        <ToolPanelHeader
          ...
          actions={<>
            ...
            <AButton variant="text" disabled={!batchInput} onClick={() => { setBatchInput(""); setBatchFilter(""); setBatchFormat(""); }}>{t.clear}</AButton>
          </>}
        />

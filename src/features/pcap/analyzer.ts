...
export function parsePcap(bytes: Uint8Array, name: string, size: number, sha256: string): PcapInfo {
  ...
  const extractedFiles = parsed.httpItems
    .map((item) => {
...
export function buildPcapExtractedFile(item: PcapHttpItem, payloadBytes: Uint8Array): PcapExtractedFile | null {
  ...
  const looksLikeFile = signature !== "Binary" || /application\/(?:octet-stream|x-msdownload|zip|pdf|json)|image\/|audio\/|video\//i.test(item.contentType) || /\.(?:zip|rar|7z|pdf|png|jpg|jpeg|gif|exe|dll|apk|jar|docx|xlsx|sqlite|db)$/i.test(item.path);
  ...
  return {
    ...
    filename: httpFilenameFromHeaders(payloadText, item.path, item.packetNo),
...
export function inspectFrame(frame: Uint8Array, packetNo: number, timestamp: string, deltaMs: number, captured: number, original: number, linkType: number | null) {
  ...
  return { packet: makePcapPacket(packetNo, timestamp, deltaMs, captured, original, protocol, source, destination, sourcePort, destinationPort, flow, info, payload.length ? payload : frame), http, dns };
}

export function makePcapPacket(no: number, timestamp: string, deltaMs: number, captured: number, original: number, protocol: string, source: string, destination: string, sourcePort: number | null, destinationPort: number | null, flow: string, info: string, previewBytes: Uint8Array): PcapPacket {
  return {
    no,
    timestamp,
    deltaMs,
...
export function buildPcapConversations(packets: PcapPacket[]): PcapConversation[] {
  ...
  for (const packet of packets) {
    ...
    const key = `${packet.protocol}|${endpointA}|${endpointB}`;
  ...
  return updatePcapConversationRisk(Array.from(map.values()).sort((a, b) => b.bytes - a.bytes).slice(0, 200));
}
...
export function buildPcapPortStats(packets: PcapPacket[]): PcapPortStat[] {
  ...
  for (const packet of packets) {
    ...
    for (const candidate of candidates) {
      const key = `${packet.protocol}/${candidate.port}`;
      const current = map.get(key) ?? { protocol: packet.protocol, port: candidate.port, packets: 0, bytes: 0, endpoints: new Set<string>() };
      current.packets += 1;
      current.bytes += packet.captured;
...
export function pcapExtractedFilesToCsv(items: PcapExtractedFile[]) {
  ...
  return [
    ["packet_no", "timestamp", "source", "destination", "host", "path", "filename", "content_type", "size", "sha256", "signature", "risk", "preview"].join(","),
    ...items.map((item) => [item.packetNo, item.timestamp, item.source, item.destination, item.host, item.path, item.filename, item.contentType, item.size, item.sha256, item.signature, item.risk.join("; "), item.preview.slice(0, 1000)].map(escape).join(","))
  ].join("\n");
}
...
export function pcapHttpToCsv(items: PcapHttpItem[]) {
  ...
  return [
    ["packet_no", "timestamp", "source", "destination", "method", "host", "path", "line", "user_agent", "content_type", "body_size", "body_sha256", "risk"].join(","),
    ...items.map((item) => [item.packetNo, item.timestamp, item.source, item.destination, item.method, item.host, item.path, item.line, item.userAgent, item.contentType, item.bodySize, item.bodySha256, item.risk.join("; ")].map(escape).join(","))
  ].join("\n");
}
...
export function pcapReportText(pcap: PcapInfo) {
  return [
    ...
    ...(pcap.extractedFiles.length ? pcap.extractedFiles.map((item) => `- #${item.packetNo} ${item.filename} ${item.signature} ${formatBytes(item.size)} sha256=${item.sha256}${item.risk.length ? ` notes=${item.risk.join(", ")}` : ""}`) : ["- none"]),
    "",
    "## IOC Preview",
  ...
  return {
    ...
    packets: pcap.packets.map(({ payloadBytes, ...packet }) => ({
      ...
      payloadBytes: {
        ...
        sha256: payloadBytes.length ? sha256Bytes(payloadBytes) : "--"
      }
    })),
...
export function pcapConversationPacketMatch(conversation: PcapConversation, packet: PcapPacket) {
  ...
  return packet.protocol === conversation.protocol &&
...
export function pcapEvidenceMatrixRows(info: {
  ...
  rows.push({
    ...
    action: info.packets.length ? "先固定 PCAP SHA256，再从高流量、异常项和明文服务开始检查。" : "确认文件格式、链路类型和截获完整性。"
  });
...
export function pcapExtractionManifestText(pcap: PcapInfo) {
  ...
  return [
    ...
    ...(pcap.extractedFiles.length ? pcap.extractedFiles.map((item) => `- #${item.packetNo} ${item.filename}; ${item.signature}; ${formatBytes(item.size)}; sha256=${item.sha256}; notes=${item.risk.join(", ") || "--"}`) : ["- --"]),
    "",
    "## Priority Events",
...
export function pcapWorkbenchBundle(pcap: PcapInfo) {
  return {
    ...
    extractedFiles: pcap.extractedFiles.map(({ bytes, ...item }) => ({
      ...item,
      bytes: { size: bytes.length, sha256: item.sha256 }
    })),
    iocs: pcap.iocs,

...
export function SqliteForensicPanel({ analysis, english }: { analysis: SqliteForensicAnalysis; english: boolean }) {
  ...
  return <div className="sqlite-forensic-workspace">
    ...
    <div className="tool-panel wide-panel sqlite-forensic-recovery-panel">
      <div className="panel-heading-row"><PanelTitle title={english ? "Recovered SQLite records" : "SQLite 残留记录恢复"} /><span className="status-pill">{analysis.recoveredRecords.length}</span></div>
      {analysis.recoveredRecords.length ? <div className="table-scroll sqlite-forensic-recovery-scroll"><table className="data-table sqlite-forensic-table"><thead><tr><th>{english ? "Confidence" : "可信度"}</th><th>{english ? "Page" : "页号"}</th><th>Offset</th><th>{english ? "Area" : "区域"}</th><th>{english ? "Mode" : "恢复方式"}</th><th>rowid</th><th>{english ? "Candidate table" : "候选表"}</th><th>Overflow</th><th>{english ? "Values" : "字段值"}</th></tr></thead><tbody>{analysis.recoveredRecords.map((record, index) => {
        const values = record.values.map((value, valueIndex) => `${valueIndex + 1}:${value.kind}=${value.value}`).join(" | ");
        const candidates = record.tableCandidates.length ? record.tableCandidates.map((candidate) => `${candidate.table} (${candidate.score}%)`).join(", ") : "--";
        return <tr key={`${record.pageNumber}-${record.offset}-${record.recovery}-${index}`} title={record.notes.join(" ")}><td><span className={`status-pill ${record.confidence === "high" ? "success" : record.confidence === "low" ? "danger" : ""}`}>{record.confidence}</span></td><td>{record.pageNumber}{record.walFrame ? ` / WAL ${record.walFrame}` : ""}</td><td><code>0x{record.offset.toString(16).toUpperCase()}</code></td><td>{record.area}</td><td>{record.recovery}</td><td>{record.rowid ?? "--"}</td><td>{candidates}</td><td>{record.overflowPages.length ? `${record.overflowPages.join(", ")} · ${formatBytes(record.overflowBytes)}` : "--"}</td><td><button className="sqlite-fragment-copy" type="button" title={values} onClick={() => void copyText(values)}>{values}</button></td></tr>;
      })}</tbody></table></div> : <div className="empty-state">{english ? "No SQLite record-shaped residue passed the conservative recovery checks." : "未发现通过保守校验的 SQLite 记录型残留。"}</div>}
      <div className="forensic-inline-note">{english ? "Recovered rows are candidates, not authoritative live records. Freeblock headers may overwrite deleted-cell prefixes; payload-only recovery therefore cannot restore rowid and table names are ranked from live schema affinity." : "恢复结果属于候选证据，不等同于当前有效记录。Freeblock 头可能覆盖已删除 Cell 的前缀，因此仅 Payload 恢复无法还原 rowid；候选表依据当前 Schema 的列数与类型亲和度排序。"}</div>
    </div>
